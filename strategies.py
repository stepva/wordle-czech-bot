...
def strategy_1(main_word, guesses, letters, known, includes, good, wrong, uniques):
    """
    První guess z uniques, druhý guess z uniques jen s možnými slovy, další normálně
    """
    guess = pick_guess_wo_altering(uniques, lts=letters)

    i = 1
    while i < 7:
        if DEBUG: print('guess: ', i, guess)
        ...
        i += 1
    if i == 7:
        if DEBUG: print('NEUHODL!', main_word)
        wrong.append(main_word)
    else:
        if DEBUG: print('UHDODL!', main_word, i)
