...
def pick_guess(guesses, lts, known=None, includes=None):
    if DEBUG: print('entering pick_guess: ', len(guesses), known, includes, first)
    ...
    if includes:
        guesses[:] = [w for w in guesses if all(len(re.findall(l, w)) == includes['exact'][l] for l in includes['exact'].keys())]
        guesses[:] = [w for w in guesses if all(len(re.findall(l, w)) >= includes['at_least'][l] for l in includes['at_least'].keys())]

    choice = random.choice(guesses)
    if DEBUG: print('exiting pick_guess: ', len(guesses), choice)
    return choice
